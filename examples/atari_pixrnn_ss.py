...
from cnslib.optimizers.score_sync import Optimizer
...
class Experiment:
    ...
    def run(self, agent_id):
        scoreboard = AgentScoreboard(self.config.num_agents, redis_params=self.config.redis_params)
    ...
    def exhibition(self, agent, environment, scoreboard):
        while True:
            best_genomes = scoreboard.fetch_top_genomes()
            genome_data = random.choice(best_genomes)
            agent.load_genome(genome_data)
...
if __name__ == '__main__':
    ...
    if config.clear_store:
        scoreboard = AgentScoreboard(config.num_agents, redis_params=config.redis_params)
        scoreboard.clear()
    ...
    if config.exhibition:
        ...
        for agent_i in range(config.num_agents):
            agent_id = agent_i + config.base_agent_id
            p = multiprocessing.Process(target=main, args=(agent_id, config,))
            p.start()
            processes.append(p)
        for p in processes:
            p.join()
